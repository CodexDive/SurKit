...
activation = "sigmoid"
loss_function = "dice and bce"
optimizer = "Adam"
lr = 3e-4
max_iteration = 2000
save_path = 'model/image_%s_unet' % bkd.backend_name
# save_path = None


net = unet.UNet(in_channels=3, out_channels=1, activation=activation)

start = time.time()
train2d(input=train_loader, evaluation=eval_loader, net=net, iterations=max_iteration,
    optimizer=optimizer, lr=lr, loss_function=loss_function, path=save_path, report_interval=10, classify=True)
