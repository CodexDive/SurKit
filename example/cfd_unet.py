...
if bkd.backend_name == 'jax':
...
elif not (bkd.backend_name == 'oneflow' and ',' in args.cuda):
    ...
    train_sampler = DistributedSampler(MyDataset(data[:train_num]))
    train_loader = DataLoader(MyDataset(data[:train_num]), shuffle=(train_sampler is None), sampler=train_sampler)
    val_sampler = DistributedSampler(MyDataset(data[train_num:]))
    val_loader = DataLoader(MyDataset(data[train_num:]), shuffle=(val_sampler is None), sampler=val_sampler)
...
max_iteration = 100
save_path = 'model/cfd_%s_unet' % bkd.backend_name
# save_path = None


net = unet.UNet(in_channels=t, out_channels=1, activation=activation)
