...
def train2d(
    ...
    optimizer = optimizer(net.parameters(), lr=lr)
    if scheduler_step:
        scheduler = flow.optim.lr_scheduler.StepLR(optimizer, step_size=scheduler_step, gamma=scheduler_gamma)
    best_loss = float('inf')
...
def train_bayesian(
    ...
    for epoch in range(iterations):
        ...
        if (epoch + 1) % report_interval == 0:
            if evaluation:
                eval_gt = flow.cat(list(eval_ground_truth.values()), dim=1)
                predict = flow.mean(flow.cat([net(flow.cat(list(eval_input_tensor_dict.values()), dim=1))
                     for _ in range(samples)], dim=0), dim=1)
